...
def getFolders():
    f = open(HOME+CURRENTSCHOOL+"_folderDirectories.txt","r")
    info = f.readlines()
    info = [l.strip('\n\r') for l in info]
    f.close()
...
class App(Frame):
    ...
    def download(self,fileID,fileName):
        if(drive.downloadTo(fileID,fileName)):
...
upImage = PhotoImage(file=HOME+"upload.gif")
downImage = PhotoImage(file=HOME+"download.gif")
